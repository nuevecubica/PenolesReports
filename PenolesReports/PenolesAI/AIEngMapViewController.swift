...
class AIEngMapPage: Page {
  func viewController(with magazine: Magazine) -> UIViewController {
    let mapVC = UIStoryboard(name: "AIEng", bundle: nil).instantiateViewController(withIdentifier: "mapViewController") as! AIEngMapViewController
    mapVC.magazine = magazine
    return mapVC
  }
}

class AIEngMapViewController: UIViewController {
